...
student_code = df.select("student_code").rdd.flatMap(lambda x: x).collect()
student_name = df.select("student_name").rdd.flatMap(lambda x: x).collect()
...
for i, id in enumerate(student_code):
    ...
    student_df.toPandas().to_csv(
        f"/data/{id}_{name}.csv", header=True
